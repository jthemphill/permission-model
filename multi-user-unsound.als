...
fact calculate_group_permissions {
    always {
        all g: Group, o: Object {
            ...
            ~(g.calculated)[o] = ordering/max[{ p: Permission {
                // Find the first ancestor going up the tree that has an explicit permission
                some ancestor: o.*parent & g.explicit[p] {
                    // There must be no explicit permission between the first ancestor and the object
                    no ~(g.explicit)[o.*parent & ancestor.^children]
                }
            } } + CannotSee]
        }
    }
...
check calculated_permission_correctness {
    // If you have explicit permission to something, that is also your calculated permission
...
} for 1 User, 2 Group, 3 Object, 2 steps
